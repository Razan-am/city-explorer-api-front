...
class App extends Component {
  ...
  handlerSubmit = async (e) => {
    ...
    let axiosResponed = await axios.get(`https://eu1.locationiq.com/v1/search.php?key=${process.env.REACT_APP_LOCATION_API_KEY}&q=${this.state.city_name}&format=json`).then(request=>{
      ...
      const moviesApi = await axios.get(`http://localhost:8000/movies/?key=1ce9a45e3f574b64af6379c6c4a8b0cc&query=${this.state.city_name}`)
    ...
    return (
        ...
        <form onSubmit={this.handlerSubmit} style={{ marginLeft: '100px', paddingTop: '20px', marginButton: '20px', display: 'block', width: '50px' }}>
          ...
          {this.state.show &&
            <div>
              ...
              <Image alt='map' src={`https://maps.locationiq.com/v3/staticmap?key=pk.0a80fd547a3c1e8574e39921b81514c5&center=${this.state.lat}&lon=${this.state.lon}&zoom=1-8`} fluid style={{ margin: '100px', width: '1000px' }} />
            </div>
          }
        {
      ...
      }
      {
        ...
        this.state.moviesData.map((item)=>{
          return(
            <Movies movieTitle={item.title} movieVotes={item.total_votes} moviePosterTitle={item.title} src={item.poster}/>
          )
        })
      }
      </>
